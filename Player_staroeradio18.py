...
class StaroeRadioPlayer:
    ...
    def setup_ui(self):
        ...
        # ========= Нижняя левая часть — Описание трека =========
        info_frame = ttk.LabelFrame(left_paned, text="Описание трека", padding="5")
    ...
    def _choose_save_dir(self):
        """Открыть диалог выбора папки для сохранения. Возвращает путь к папке SRPlayer_downloads
        ...
        srplayer_dir = os.path.join(chosen, "SRPlayer_downloads")
        try:
            os.makedirs(srplayer_dir, exist_ok=True)
        except Exception as e:
            self.log(f"❌ Ошибка создания папки SRPlayer_downloads: {e}")
            return None
    ...
    def save_m3u(self):
        ...
        srplayer_dir = self._choose_save_dir()
        if not srplayer_dir:
            return
        ...
        query_dir = os.path.join(srplayer_dir, safe_query)
        try:
    ...
    def _save_track_to_m3u(self, track):
        ...
        m3u_filepath = os.path.join(srplayer_dir, f"{safe_name}.m3u")
    ...
    def _download_mp3(self, items, is_single=False):
        ...
        if is_single:
            # Одиночный трек сохраняется прямо в SRPlayer
            download_dir = srplayer_dir
        else:
            ...
            download_dir = os.path.join(srplayer_dir, safe_query)
    ...
    def _restore_playback(self, track, position):
        ...
        try:
            ...
            self.log(f"▶ Воспроизведение восстановлено с позиции {position}мс: {track['title'][:40]}")
        except Exception as e:
    ...
    def load_colors_config(self):
        ...
        default_colors = {
            "results_tags": {
                "number": {
                    "foreground": "#979695",
                    "description": "Номер трека (серый)"
                },
                "title": {
                    "foreground": "#61A0F3",
                    "description": "Название трека (синий)"
                },
                "date_header": {
                    "foreground": "#FFD54F",
                    "description": "Заголовок даты в программе (жёлтый)"
                },
                "time_text": {
                    "foreground": "#888888",
                    "description": "Время передачи в программе (серый)"
                },
                "selected": {
                    "background": "#1E3A8A",
                    "foreground": "#FFFFFF",
